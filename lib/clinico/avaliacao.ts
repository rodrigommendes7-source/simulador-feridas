...
import { gerarTodasPerguntasJustificacao, gerarPerguntaJustificacao } from "./motor-justificacao.ts";
import { avaliarAnotacao } from "./anotacoes-avaliacao.ts";
import type {
  RevisaoTentativa,
  IdAplicacao,
  EntradaTentativa,
...
function criarSeccao(
  ...
  return { id, titulo, pontuacao: 0, pontuacaoMaxima, itens: [] };
}
...
function construirSeccaoTratamento(modelo: ModeloCaso, tentativa: EntradaTentativa) {
  ...
  const regraEspeciais = modelo.regrasAvaliacao.filter((rule) => rule.alvo === "tratamento");
  ...
  for (const tratamento of selected) {
    ...
    if (objetivosCorrespondentes.length === 0) {
      ...
      if (isUniversalCleanser) {
        adicionarItemTratamento(
          ...
          tratamento.idsTemasAprendizagem
        );
        continue;
      }
      adicionarItemTratamento(
    ...
    adicionarItemTratamento(
      ...
      [...objetivoEscolhido.idsTemas, ...tratamento.idsTemasAprendizagem]
    );
  }
...
function construirSeccaoAplicacao(modelo: ModeloCaso, tentativa: EntradaTentativa) {
  ...
  const regraEspeciais = modelo.regrasAvaliacao.filter((rule) => rule.alvo === "aplicacao");
